// Array of dummy data representing sensor readings
...
// Define a function to calculate overall average, minimum, maximum, and other statistics for a given data type
export const GetOverallAverage = (arrayOfData, type) => {
  ...
  // Determine the data type based on the input
  if (type === "Temperature") {
    unit = '°C';
    typeKey = 'temperature';
    ...
    unit = 'L';
    typeKey = 'levels'; // Assuming 'levels' represents the combined chemical levels
  }

  // Loop through the array of data to calculate statistics
  for (let x = 0; x < arrayOfData.length; x++) {
    if (typeKey === "levels") {
      // Calculate statistics for combined chemical levels
      if (x === 0) {
      ...
      // Calculate statistics for other data types (e.g., Temperature or Pressure)
      if (x === 0) {
  ...
  return { max, min, average, levelMin, levelMax, avgLevel, unit };
};